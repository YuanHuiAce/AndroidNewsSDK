 ...
 */
public class NewsDetailADRequestPost<T> extends JsonRequest<T> {

    private HashMap mHeader = new HashMap();
    public NewsDetailADRequestPost(String url, String requestBody, Response.Listener<T> listener, Response.ErrorListener errorListener) {
    ...
    protected Response<T> parseNetworkResponse(NetworkResponse response) {
        try {
            ...
            if ("2000".equals(code)){
                ArrayList<NewsFeed> result = GsonUtil.deSerializedByType(jsonObject.getJSONArray("data").toString(), new TypeToken<ArrayList<NewsFeed>>() {
                }.getType());
